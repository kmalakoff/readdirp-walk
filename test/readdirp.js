...
var test     = require('tap').test
  ...
  , util     = require('util')
  , net      = require('net')
  , readdirp = require('../index.js') // KM
  , root     = path.join(__dirname, 'bed') // KM
  , totalDirs          =  6
  , totalFiles         =  12
  , ext1Files          =  4
  , ext2Files          =  3
...
test('\nreading root without filter', function (t) {
  ...
  readdirp(opts(), function (err, res) {
    console.log(err, res.directories.length, totalDirs, res.files.length, totalFiles)
...
// KM: skip
true || test('resolving of name, full and relative paths', function (t) {
  var expected = {
